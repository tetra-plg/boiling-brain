...
- **`validate-wiki.py` enforces per-type frontmatter** (`.claude/rules/frontmatter.md` was documentation-only until now): `type: decision` requires `status` (closed enum `pending|accepted`); `type: source` requires `source_path`, `source_sha256` (64-char lowercase hex — rejects `TODO`, slugs, truncated hashes) and `ingested`; a non-null `verdict` must be in `validated|invalidated|partial` and carry `verdict_date` + `verdict_evidence`. Unknown types (e.g. `index`) are unaffected. **Vocabulary change**: v1.1.0 described an open-ended `status` and `verdict` surface; v1.3.0 closes both to the enums above (motivation: avoid narrative drift in ADR status, enforce verdict precision for reality validation); `type` remains open, allowing future page taxonomies without schema updates. **Migration note**: vaults may need one backfill pass — run `python3 scripts/wiki-maint/validate-wiki.py --warn-frontmatter-types` to triage without failing CI (the transitional flag downgrades only these new checks to stderr warnings), backfill, then drop the flag; see `scripts/migrations/v1.3.0.md` Part A. (#104)
- **Format parity between the deposit channel and the ingestion engine** (`drop_file_to_raw` + docx/pptx twins): `/ingest` consumed markdown/text, PDF, png/jpg images natively and video through `/ingest-video`, but `drop_to_raw(subfolder, filename, content)` only carried **text** — an MCP client without a terminal (Claude Desktop, Claude Cowork) had no sanctioned way to archive a binary into `raw/`, and docx/pptx were readable nowhere in the chain. Three additions close the asymmetry. (1) New MCP tool **`drop_file_to_raw(source_path, subfolder)`**: server-side copy of a local file into `raw/<subfolder>/` (the stdio server runs on the vault machine), same `cache/.pending-ingest` signalling as `drop_to_raw`. Guardrails: the source is resolved through symlinks and must sit under an allowed root (`$HOME` by default, `LLMWIKI_DROP_SOURCE_ROOTS` — `os.pathsep`-separated — to override), must carry an extension the engine consumes, and must not already live in `raw/`; the destination is confined to `raw/` on both segments; an existing target is never overwritten (the original is copied, not moved). (2) New **`scripts/convert-doc.sh`**: pandoc-backed markdown twin of a `.docx`/`.pptx` sitting in `raw/`, written next to the original (`brief.docx` → `brief.docx.md`), never overwritten — a second run is a no-op that still prints the path, so `/ingest` can chain unconditionally. Exit codes `0/1/2/3` (twin available / bad argument / pandoc missing or too old / conversion failed); pptx input requires pandoc ≥ 3.0, docx works on 2.x. `/ingest` step 1 chains it on every in-scope docx/pptx and ingests the **twin**; step 3 instructs the agent to declare `source_path` = the binary original with its `source_sha256` and to list the twin in `covered_paths`, so the archived original stays the hash-indexed source of truth (no composite hash needed). The headless guard allowlists the script with a charset-anchored regex. (3) Documentation resynced: `README.md` (15 tools exposed, 12 of them read), `docs/mcp-tiered-loading.md` (write-side block + a deposit/ingestion format-parity table), `docs/cowork-project-instructions.md`, `CLAUDE.md.tpl` and `.claude/rules/raw-vs-cache.md` (the twin is the second sanctioned script write into `raw/`, alongside frame promotion — a rendition, never a source). `scripts/mcp/setup-mcp.sh` gained the new tool in its global `CLAUDE.md` block **and** moved its freshness probe to `drop_file_to_raw`: it still probed for `list_domains`, present since v1.2.1, so every already-updated vault looked current and the block would have been frozen. Machine prerequisite (pandoc) and MCP reload are covered by `scripts/migrations/v1.3.0.md` Part C — heads-up only, never blocking. 16 new unit tests. (#112)
- **`docs/cowork-project-instructions.md` — project-instructions template for Claude Desktop / Cowork**: the MCP server is registered at user scope with a stdio config shared by both apps, so a vault is readable from either out of the box — but neither client carries the framing Claude Code gets from `protect-raw.sh`, the slash commands and the domain-expert agents. Observed failure modes when designing a non-technical-curator setup: untiered page dumps, answers with no cited pages, no deposit discipline, and accidental direct writes when the vault folder is exposed as a working folder. The new template restores that framing client-side in five paste-ready blocks — anchor (knowledge lives in the brain, the vault folder is never a working folder), reading ritual (tiered descent, cite per point, **confidence grading per point**: 100 % backed by cited pages / 80 % solid deduction with the missing piece named / ≤ 50 % assumption stated conditionally), production ritual, deposit ritual (offer proactively, `deposited/` copy is purgeable since `raw/` is authoritative, a new version is a new deposit), and calibrated confidentiality (web research allowed, outgoing queries stripped of identifying details). Placeholders documented, cross-referenced from `README.md` and `docs/mcp-tiered-loading.md`. Documentation-only — no tool, workflow or frontmatter change. (#113)
...
- **`/sync-repos` perimeter revisions**: widening `paths:` in `tracked-repos.config.json` had no effect until an unrelated upstream commit — the sync keyed solely on the SHA and answered `SKIPPED`, silently leaving the manifest and the disk in disagreement. Every snapshot has always recorded its capture perimeter in `.sync-meta.json`; the sync now compares it (set-wise) with the manifest and captures a **revision** `<dest>/<shortsha>-rN/` on any change, leaving the previous snapshot untouched. The `CREATED <path>` stdout contract is unchanged (the path may carry `-rN`; explanations go to stderr as `note:` lines), `/ingest` stays quiet on unchanged files via content coverage (#88), and hand-made snapshots without a recorded perimeter are never auto-revisioned. First test coverage of the script (`scripts/wiki-maint/test_sync_repos.py`, stubbed `gh`). (#106)
- **Async ingestion — `ingest_start` / `ingest_status` / `ingest_cancel` (15 → 18 tools)**: the sync `ingest()` blocks on the whole headless run (bounded by its 600 s watchdog), but MCP clients enforce their own tool-call timeout — observed at 60 s on Claude Desktop / Cowork, killing a real run with no report and no way to tell a failed run from a slow one; the escape hatch planned in #62 ("add an async mode if real usage shows durations incompatible with third-party MCP clients") is now exercised. New dependency-free job manager `scripts/mcp/ingest_jobs.py` (state under `cache/ingest-jobs/<job_id>.json`, child stdout/stderr captured to files — no PIPE deadlock): `ingest_start(path, domain_hint="")` validates exactly like `ingest()` (never spawns on invalid input) then launches the same guarded `claude -p "/ingest … --headless"` via non-blocking `Popen`; `ingest_status(job_id)` returns `running` with elapsed time, the byte-identical sync report (machine-parseable `## Pages` block) on completion, an error with a stderr excerpt, or a timeout notice (same 600 s bound, the watchdog is enforced when polled — a crashed/hung poller means no one ever hits the bound); `ingest_cancel(job_id)` terminates the child (SIGTERM → SIGKILL after 5 s) and is idempotent on finished jobs. One job at a time (a headless run owns `wiki/log.md`, the radar and the index — concurrent runs would interleave their journaling). Hardening from the final review pass: state writes are atomic (`os.replace`), and a state file that fails to parse (partial write, corruption) is skipped rather than raised, so one bad file can never brick `ingest_start`; after an MCP server restart, a job still marked `running` has no in-process handle to trust and its persisted pid may already have been recycled by an unrelated process, so it is **never signaled** — it is finalized as an explicit error pointing at `wiki/log.md` instead of a guessed exit code; `ingest_cancel` on a child that already exited before being polled finalizes and returns its real report rather than discarding it under a `cancelled` stamp. Sync `ingest()` unchanged (validation factored out, error strings byte-identical). 18 unit tests, run against real short-lived child processes (`scripts/mcp/test_ingest_jobs.py`). Docs resynced: `README.md`, `docs/mcp-tiered-loading.md`, `setup-mcp.sh` block (+ freshness probe moved to `ingest_start`, per the #112 convention). (#124)
- **CI `unittests` job**: the whole Python suite (142+ tests: scan-raw, validate-wiki, sync-repos, format-md, archive-radar, MCP core/CLI/guard) only ever ran on a maintainer's machine — a regression in any of them merged green (spotted during the #106 final review). New job in `lint.yml` on every push/PR (excluded from the weekly link-check schedule): Python 3.12, installs `pyyaml` + `fastmcp` so the MCP server tests run for real (27 skips otherwise), then `unittest discover` over `scripts/wiki-maint/` and `scripts/mcp/`. README's CI section updated. (#119)

### Changed

- **`sync-repos.sh` fully English**: remaining French output labels (prereq errors, `gh api`/clone failures, "no listed path exists") and header comments violated the EN-only policy for propagated files — missed by the v1.2.1 i18n pass (#95) and only partially covered by the #106 rewrite. String-only change: the machine-parseable stdout protocol (`CREATED` / `SKIPPED` / `ERROR` / `note:` tokens) is untouched. (#120)

### Fixed

- **Cowork deposit ritual: no trigger, missing cloud → disk bridge, stale ingestion wording**: observed on a real cloud Cowork session — a PDF attached as the session's very first message (the primary source of the whole analysis that followed) was offered for deposit only in the third reply, as an aside. `docs/cowork-project-instructions.md` §4 is rewritten around an explicit trigger (the arrival of a document IS the trigger: deposit offered in the first reply after it appears, naming tool, target subfolder and `domain_hint` — never as a closing aside), an ordering rule (primary source first, derived documents once stable, a still-iterating draft waits — `raw/` is immutable), the **mandatory cloud bridge** (a cloud attachment lives in the session container where the server-side `drop_file_to_raw` cannot see it: commit it into the project working folder on the vault machine first — described functionally, current client tool names kept only as a dated example), and the current async flow (`ingest_start` + `ingest_status` polling replaces the stale "ingestion happens later, in batch" wording; `domain_hint` effectively mandatory). Error-as-instruction on the tool side, the part with the longest reach (a client with no hook mechanism still reads the error): `drop_file_to_raw`'s out-of-root and file-not-found refusals now name the bridging step instead of only suggesting `LLMWIKI_DROP_SOURCE_ROOTS` (actively misleading for a cloud session), its description distinguishes desktop clients (direct call on a local path — nominal, unchanged) from cloud sessions, the `ingest`/`ingest_start` descriptions state what happens without a `domain_hint`, and the headless report's `needs-human-triage` section now ends with an explicit fix line (`retry with ingest_start(path, domain_hint=<slug>) — slugs via list_domains()`) instead of reading as a silent no-op. Migration Part D: re-paste §4 into existing Desktop/Cowork projects (the pasted copy is out of file-propagation's reach). (#135)
- **`setup-mcp.sh` registers the server for Claude Code only — Desktop/Cowork config never written**: the script registered via `claude mcp add -s user` (which writes `~/.claude.json`, read by the Claude Code CLI alone) while `docs/cowork-project-instructions.md` claimed one run covered Claude Desktop and Cowork too — the first setup step of the no-terminal curator flow (#113) failed silently for every non-Claude-Code user. A new stdlib-only helper (`scripts/mcp/register-desktop-config.py`, called by `setup-mcp.sh`, covered by 6 unit tests) now merges the same server entry (same resolved interpreter, script and `WIKI_PATH`) into `claude_desktop_config.json` (macOS/Windows/Linux paths, `CLAUDE_DESKTOP_DIR` override): other `mcpServers` entries and top-level keys are content-preserved, the write is atomic, a re-run is idempotent (update-in-place), an existing file with invalid JSON is left untouched and reported without aborting the rest of the setup, and a missing Claude app directory produces a clear skip notice without affecting the Claude Code registration. The doc's Setup step 1 and the v1.3.0 migration Part C2 now describe the actual behaviour (restart the app after the run). (#133)
- **Headless ingest guard vs the `Task` → `Agent` tool rename**: the guard's allowlist pinned `Task`, the former name of Claude Code's subagent-spawning tool; on current Claude Code the spawn call is denied and the headless ingest silently falls back to running in the main context — plausible output, no error anywhere (observed on a real run, 2026-08-01). The allowlist now reads `Task|Agent|ToolSearch` (`Task` kept for older clients), the `Write`/`Edit` scope additionally allows `.claude/agent-memory/*` so domain experts accumulate memory on headless runs too (previously interactive-only, by accident not by design), and the catch-all deny message now tells the model to state the degradation explicitly in its report and `wiki/log.md` entry (loud-fail: the deny itself is the only reliable detection point for the next upstream rename). New smoke suite `scripts/mcp/test_ingest_headless_guard.py` pins the tool vocabulary and the Write/Bash scopes — with #119, the next rename shows up as red CI instead of a silently degraded run. (#125)
- **`/lint` structural pass no longer disk-checks `sources:`**: the _Stale raw sources_ bullet told the linter that every `sources:` entry must exist on disk under `raw/` — but `sources:` is documented (`.claude/rules/frontmatter.md`) as a wiki-to-wiki wikilink, never a disk path: an LLM following the instruction to the letter reproduced the 100 %-false-positive class #103 removed from `scan-raw.py --orphans`. The disk-existence check now covers `source_path:`/`covered_paths:` only, and a dedicated bullet checks the actual contract: each `sources:` wikilink must resolve to an existing wiki page. (#110)
- **v1.1.0 migration no longer claims "12 tools" as current**: `scripts/migrations/v1.1.0.md` is a live document — a vault still on v1.0.x running `/update-vault` today was told the server "now exposes 12 tools" while the `setup-mcp.sh` it executes registers the current surface. Both statements are now time-stamped ("12 as of v1.1.0; 14 since v1.1.1"), which stays true by construction as the surface keeps growing. Migration steps and re-run semantics untouched. (#111)
- **`drop_to_raw` could write outside `raw/`** (found while factoring its destination guard for #112): the traversal check compared resolved paths with `str.startswith`, so a `subfolder` of `../rawbis` — which resolves to `<vault>/rawbis`, a **sibling** of `raw/` whose path string legitimately starts with `<vault>/raw` — passed the guard and the file landed outside the immutable archive (reproduced, then covered by a regression test). Both segments are now compared component-wise (`child == parent or parent in child.parents`), which closes the whole sibling-prefix class rather than the one payload. Present since the tool shipped in v1.1.0; `drop_file_to_raw` inherits the fixed guard. (#112)
...
- **`/update-vault` is now MCP-aware**: propagating MCP-stack files used to leave the running server on the previous code with no signal, while the v1.1.0 migration re-offered `setup-mcp.sh` on **every** update regardless of whether anything MCP had changed. Step 5 now records which MCP files were actually **staged**, and a new conditional **step 7** offers `setup-mcp.sh` only when `scripts/mcp/mcp-wiki.py`, `wiki_core.py` or `setup-mcp.sh` was propagated — then prints an explicit **reload instruction** when the _server code_ changed, since re-registration does not reload the running process. `wiki-cli.*`, the headless guard and the MCP tests never trigger it. The `force-rerun: true` flag was removed from `scripts/migrations/v1.1.0.md` accordingly: vaults that never applied it still receive it through the normal path, and those that did stop seeing the systematic prompt. Offered, never run silently — `setup-mcp.sh` mutates user-global files (`~/.claude/settings.json`, `~/.claude/CLAUDE.md`). (#90)
- **MCP stack translated to English**: `scripts/mcp/` shipped French output — including `wiki_core.<tool>_md()` markdown and `mcp-wiki.py` tool return values read by the LLM on every call, `ingest-headless-guard.sh` deny messages, and `setup-mcp.sh` console + the `CLAUDE.md` instructions block it injects. All of it is now English, matching the EN-only policy for propagated files (wiki _content_ still follows `vault_language`; only the infrastructure layer is neutralised). String-only change: tool names, MCP schema, JSON shapes and exit codes are unchanged, and the dynamic CLI↔core parity test is untouched. On the next `/update-vault`, step 7 (#90) detects the propagated MCP files and offers `setup-mcp.sh`, which refreshes the global `CLAUDE.md` block to English. (#95)
...
- **`scripts/wiki-maint/scan-raw.py`**: `/sync-repos` re-snapshots no longer flood `/ingest` with phantom `NEW` files. `scan-raw` now also covers snapshot files **by content** (sha256), scoped to the same `(dest, relative-path)` lineage via each snapshot's `.sync-meta.json`: a re-snapshot whose documented subtree is byte-identical to a covered version is `SKIP`, not `NEW`, and a partial change flags only the file whose content actually changed (0 doc change → 0 NEW, 1 file changed → 1 NEW). Hashes are cached in `cache/.hash-cache.json` (keyed by mtime+size) so immutable snapshots are hashed once — this also speeds up the existing `MODIFIED` detection. No frontmatter change, no migration; `sync-repos.sh` is untouched. (#88)
- **`scripts/wiki-maint/format-md.py` version guard**: the script uses `glob(..., include_hidden=True)` (a Python 3.11+ argument) with no guard, so on an older `python3` it died with an opaque `TypeError` that never mentioned the version requirement. It now fails fast with an actionable message naming both the required (`3.11+`) and found (e.g. `3.9.6`) versions and exits non-zero, before `expand()` reaches the offending call. No behaviour change on 3.11+. (#97)
