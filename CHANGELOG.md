...
- **`scripts/migrations/v1.1.2.md`**: single interactive, idempotent `/update-vault` migration for v1.1.2 — Part B inserts the Domain orientation section into existing `<domain>-expert.md` agents and adds the `wiki-cli.sh` allowlist rule to the vault's `.claude/settings.json`; Part A coordinates the scan-raw rewrite propagation and runs a health check (see Fixed).
