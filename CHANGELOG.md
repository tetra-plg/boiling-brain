...
- **`/update-vault` — auto-refresh of MCP setup post-propagation**: when `scripts/mcp/setup-mcp.sh` or `scripts/mcp/mcp-wiki.py` are part of the propagated files AND the `v1.1.0` migration is not in the chain (i.e. the vault is already at v1.1.0), `/update-vault` now offers to re-run `setup-mcp.sh` via `AskUserQuestion` right after the propagation commit. Closes the drift on vaults already migrated when the MCP script ships changes that affect external side-effects (claude mcp registration, Stop hook, global CLAUDE.md block).
